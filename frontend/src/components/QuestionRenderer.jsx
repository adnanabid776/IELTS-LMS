    ...
    <>
      ...
      <div className="mb-4">
        <div className="flex items-start gap-4">
          <span
            className={`w-10 h-10 flex-shrink-0 rounded-xl flex items-center justify-center font-bold text-sm shadow ${answers[question._id]
                    ...
                    <span key={idx}>
                      ...
                      {idx < parts.length - 1 && (
                        <input
                          ...
                          placeholder="..."
                          className={`inline-block w-40 sm:w-48 mx-1 px-3 py-1 border-b-2 rounded-md text-sm focus:outline-none transition-all duration-200 align-baseline ${answers[question._id]
        ...
        question.options && (
          <div className="ml-14 space-y-2">
            {question.options.map((option, optIndex) => (
              <label
                key={optIndex}
                className={`flex items-center p-4 rounded-xl cursor-pointer transition-all duration-200 border-2 ${answers[question._id] === option
                    ? "border-blue-500 bg-gradient-to-r from-blue-50 to-indigo-50 shadow-md"
                    : "border-gray-200 hover:border-blue-300 hover:bg-gray-50"
                  }`}
              >
                <span
                  className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold mr-4 transition-all ${answers[question._id] === option
                ...
                />
                <span
                  className={`flex-1 ${answers[question._id] === option
        ...
        question.questionType === "yes-no-not-given") && (
          <div className="ml-14 flex flex-wrap gap-3">
            {(question.questionType === "yes-no-not-given"
              ...
              <label
                key={option}
                className={`flex items-center px-6 py-3 rounded-xl cursor-pointer transition-all duration-200 border-2 ${answers[question._id] === option
                    ? idx === 0
              ...
              >
                ...
                />
                <span
                  className={`font-medium ${answers[question._id] === option
                      ? idx === 0
                        ? "text-green-700"
                        : idx === 1
                          ? "text-red-700"
        ...
        <div className="ml-14 space-y-3">
          ...
          {question.options.map((option, idx) => {
            ...
            return (
              <label
                key={idx}
                className={`flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 group ${isSelected
              ...
              >
                ...
                <div className="flex gap-3">
                  <span
                    className={`font-bold ${isSelected ? "text-blue-600" : "text-gray-500"
                      }`}
                  >
                    {String.fromCharCode(65 + idx)}.
                  </span>
        ...
        <div className="ml-14 space-y-3">
          <div className="relative">
            <input
              ...
              className={`w-full px-5 py-3 border-2 rounded-xl focus:outline-none transition-all duration-200 ${answers[question._id]
          ...
          <div className="ml-14 space-y-4">
            ...
            <div className="relative">
              <select
                ...
                className={`w-full px-5 py-3 border-2 rounded-xl focus:outline-none transition-all duration-200 appearance-none bg-white ${answers[question._id]
      ...
      {/* Answer Input - Matching Headings / Information / Features */}
      {(question.questionType === "matching-headings" ||
        question.questionType === "matching-information" ||
        question.questionType === "matching-features") && (() => {
          const hasItems = question.items && question.items.length > 0;
          const hasFeatures = question.features && question.features.length > 0;
          const hasOptions = question.options && question.options.length > 0;

          // COMPOSITE MODE: Has items array (sub-questions with dropdowns)
          if (hasItems || hasFeatures) {
            return (
              <div className="ml-14 space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                  ...
                  <ul className="space-y-1">
                    {hasFeatures
                      ...
                      : hasOptions && question.options.map((opt, idx) => (
                        <li key={idx} className="text-sm text-gray-600">
                          <span className="font-bold mr-2 text-gray-800">
                            {question.questionType === "matching-headings"
                              ? ["i","ii","iii","iv","v","vi","vii","viii","ix","x","xi","xii","xiii","xiv","xv"][idx] || idx + 1
                              : String.fromCharCode(65 + idx)}
                            .
                          </span>
                          {opt}
                    ...
                    >
                      <div className="flex gap-4">
                        <div className="flex-shrink-0 w-8 h-8 bg-blue-100 text-blue-700 rounded-lg flex items-center justify-center font-bold">
                          {question.questionType === "matching-headings" 
                            ? (item.label || (question.questionNumber + idx).toString())
                            : (item.label || (idx + 1).toString())}
                        </div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                const currentAnswers =
                                  answers[question._id] || {};
                                const itemKey = question.questionType === "matching-headings"
                                  ? (item.label || (question.questionNumber + idx).toString())
                                  : (item.label || (idx + 1).toString());
                            ...
                            >
                              ...
                              {hasFeatures
                                ...
                                : hasOptions && question.options.map((opt, optIdx) => (
                                  <option
                                    key={optIdx}
                                    value={String.fromCharCode(65 + optIdx)}
                                  >
                                    {question.questionType === "matching-headings"
                                      ? ["i","ii","iii","iv","v","vi","vii","viii","ix","x","xi","xii","xiii","xiv","xv"][optIdx] || optIdx + 1
          ...
          // STANDALONE MODE: No items — single question, just pick a paragraph letter
          return (
            <div className="ml-14 space-y-3">
              {/* Show options list if available */}
              {hasOptions && (
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                  <h5 className="font-bold text-gray-700 mb-2">
                    {question.questionType === "matching-headings"
              ...
              {/* Simple dropdown to pick a letter */}
              <div className="flex items-center gap-3">
                <label className="text-sm font-semibold text-gray-600">
                  Select Paragraph:
                </label>
                <select
                  ...
                  className={`px-4 py-2 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${
                    answers[question._id]
          ...
          <div className="ml-14 space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                className={`px-4 py-2 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all ${answers[question._id]
                    ? "border-green-400 bg-green-50"
                    : "border-gray-300"
                  }`}
              >
                            ...
                            if (match) {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <input
                                    type="text"
                                    className={`w-32 px-2 py-1 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${answers[question._id]?.[
                                        answerIndex
                                      ]
                                      ...
                                      {isBlank && (
                                        <span className="whitespace-nowrap inline-flex items-center gap-1 mx-2">
                                          ...
                                          <input
                                            ...
                                            className={`w-32 sm:w-48 px-2 py-1 border-b-2 focus:outline-none bg-transparent transition-colors font-bold ${currentVal ? "border-green-500 text-green-700" : "border-gray-400 focus:border-blue-600 text-blue-900"
                                              }`}
                                            placeholder="..."
                                          />
                                        </span>
      ...
      {/* Answer Input - Writing Task */}
      {question.questionType === "writing-task" && (
        <div className="ml-14 mt-4 w-full max-w-4xl">
          {question.imageUrl && (
            <div className="mb-6 object-contain overflow-hidden rounded border border-gray-200 flex justify-center bg-white p-2">
              <img
          ...
          <div className="mt-4 shadow-sm border border-gray-300 rounded-lg bg-white overflow-hidden flex flex-col">
            <div className="bg-gray-100 px-4 py-3 border-b border-gray-300 flex justify-between items-center">
              <span className="font-semibold text-gray-700">✍️ Essay Editor</span>
              {(() => {
                const val = answers[question._id] || "";
                const wordCount = val.trim() ? val.trim().split(/\s+/).length : 0;
                const targetWords = question.wordLimit || 150;
                const color = wordCount >= targetWords ? "text-green-600" : "text-gray-500";
            ...
            <textarea
              ...
              className="w-full px-5 py-4 focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[350px] resize-y text-gray-800 leading-relaxed font-serif"
            ></textarea>
          </div>
        </div>
      )}
