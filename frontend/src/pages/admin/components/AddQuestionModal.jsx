...
const AddQuestionModal = ({ sections, onClose, onSuccess }) => {
  ...
  React.useEffect(() => {
    if (
      ...
      // For others, we keep the existing A, B, C logic.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        ...
        const itemTypes = ["form-completion", "flow-chart-completion", "diagram-labeling", "map-labeling"];
        ...
        } else if (itemTypes.includes(formData.questionType)) {
      ...
      // Handle Summary/Sentence Config Options Cleanup
      if (
                  ...
                  <div className="space-y-4">
                    {formData.items.map((item, index) => (
                      <div key={index} className="flex gap-3 items-start">
                        ...
                        >
                          {formData.questionType === "matching-headings"
                            ? (item.label || (parseInt(formData.questionNumber || 0) + index).toString())
                            : item.label}
                        </div>
                        <div className="flex-1 space-y-2">
