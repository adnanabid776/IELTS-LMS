...
const authMiddleware = async (req, res, next) => {
  try {
    ...
    if (user.activeSessionToken !== tokenHash) {
      console.log(`❌ Session invalidated for user ${user.email}. Token mismatch.`);
      console.log(`   Stored hash: ${user.activeSessionToken ? user.activeSessionToken.substring(0, 10) + '...' : 'null'}`);
